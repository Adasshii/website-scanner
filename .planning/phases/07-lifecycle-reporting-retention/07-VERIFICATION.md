...
human_verification_resolved:
  - test: "Redeploy production (`npx vercel --prod`) so the paginated `getReportingData()` ships, before prospect/outreach/scan volume crosses 1000 rows."
    ...
    outcome: "DONE 2026-08-03. Joshua authorised the deploy in-session; run from the repo root. Deployment dpl_Hj47paoR7pLYNS2jxgvNjuYtxEzT, readyState READY, target production, aliased to scan.adashi.io. Live surfaces re-checked after the deploy: /api/cron/retention 401, /api/admin/reporting 401, /admin 200. Cron registration independently re-read against the Vercel API (v9 project `crons` and v13 deployment `crons`, token at ~/Library/Application Support/com.vercel.cli/auth.json): all five registered, `disabledAt: null`, `crons.deploymentId` pointing at the live deployment, and zero day-of-month expressions — the condition that silently dropped /api/cron/retention on the first deploy. `/api/cron/retention` is registered at `0 3 * * *`, matching D-7-20-as-superseded. Superseded by dpl_55Fp6dFhbJg9pbKKZXHJdxJdtFAf (2026-08-03 16:34), which additionally ships 0b33191; the cron read above is against that deployment."
