...
- [ ] **CMP-01**: A suppression list in Supabase is the source of truth for who must not be contacted
- [ ] **CMP-02**: Suppression is checked immediately before dispatch, not at draft time, because state can change while a draft waits
- [ ] **CMP-03**: Suppression matches on both email address and domain
- [ ] **CMP-04**: An unsubscribe endpoint writes to the suppression list synchronously before returning success, and is idempotent
- [ ] **CMP-05**: Unsubscribes take effect permanently and by the very next send cycle, with no delay language
- [ ] **CMP-06**: No code path can re-add a suppressed record without an explicit, logged manual override
