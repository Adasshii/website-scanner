...
| **Outreach is sent manually from Joshua's own mailbox, with no third-party dispatch provider** (2026-08-04, supersedes the open-channel position) | At 10 to 50 sends per week the acceptable-use problem is removed rather than answered, and the Phase 2 suppression spine plus the Phase 6 human gate stay intact. Every transactional ESP evaluated (Mailgun, SendGrid, Brevo, Amazon SES, Resend) prohibits the activity outright, so switching providers was never a fix. See `.planning/research/SEND-CHANNEL.md` | ✓ Good (closed the provider half of the Phase 8 gate) |
