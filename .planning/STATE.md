...
Last activity: 2026-08-06: Two quick tasks fixed the scanner-service deploy, which had been broken in two separate ways since `14af0a9`: 260806-kbi fixed the Docker BUILD, 260806-luk fixed the container BOOT. Migration 020 was applied to production through the Supabase Dashboard SQL Editor, clearing the deferred DDL step, and `main` was pushed, so Vercel now serves Phase 8 and Railway is running a green scanner-service deployment for the first time since `14af0a9`. Phase 8 itself is unchanged from 2026-08-05: executed, 3/3 plans, Prepare/Mark as sent/the immutable audit record/the isolation guard all ship, and every Prepare on a real draft refuses with `legal-basis-unset`, which is the designed state until counsel supplies `legal_regimes.legal_basis` and `article_14_notice_approved`.
