...
export function CostEstimateSection({ costEstimate }: CostEstimateProps) {
  const t = useTranslations("costEstimate");
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6 sm:p-8 mb-8">
      <h2 className="font-display text-lg sm:text-xl text-amber-900 mb-4">{t("heading")}</h2>
      ...
      <p className="text-amber-800 text-base sm:text-lg leading-relaxed mb-5">
        {t("explanation", { percent: perHundred })}
      </p>
          ...
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline gap-2">
                <span className="font-semibold text-amber-900 text-sm">{factor.name}</span>
                <span className="text-amber-700 text-xs font-medium">
                  {t("impactSuffix", { percent: factor.percentImpact })}
                </span>
              </div>
              <p className="text-amber-700 text-sm leading-relaxed">{factor.explanation}</p>
            </div>
          </div>
        ))}
      </div>

      <p className="text-amber-600 text-xs">{t("footnote")}</p>
    </div>
  );
}
